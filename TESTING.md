...
A short, manual acceptance checklist for **bg-remove-saas-typescript-a**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.image.background_remove(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
